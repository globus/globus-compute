...
The |ShellFunction| class allows for the specification of a command string, along with
runtime details such as a run directory, per-task sandboxing, walltime, etc and returns a
|ShellResult|. |ShellResult| encapsulates the outputs from executing the command line string
