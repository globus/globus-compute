...
access to execute their code. Like any Lambda runtime, Compute endpoints are what's
ultimately responsible for executing a function's code.

To manage Compute endpoints, an endpoint administrator installs `the open source
package <https://github.com/globus/globus-compute/tree/main/compute_endpoint>`_ on a
Linux-based machine they want to share with others, then uses the
``globus-compute-endpoint`` CLI to create endpoints, configure them, and register them
with the hosted Compute services. Once an endpoint is registered, its given a UUID that
users can use to submit to that endpoint. Generally any function can be run on any
endpoint that a user has access to. :doc:`For security reasons
<../endpoints/security_posture>` endpoints are only accessible to their owners by
default, but they can easily be opened up as needed.
...
.. code-block:: python
  :caption: Simple Lambda function definition
...
.. code-block:: shell
  :caption: Registering this to Lambda

  zip function.zip module.py
...
The Compute SDK handles packaging up the function transparently to users. That said,
the surrounding context is **not** included---so in Compute, function developers need
to add imports **inside the function**. Since Compute functions are regular Python
functions, they don't have to follow any particular API or contract except anything
appropriate for the workload---in this case, we simply take in the array.

While this style covers the vast majority of usecases, sometimes a Lambda-style "module
with a handler function" approach is needed. Compute supports this mode as well:
...
.. code-block:: python
  ...
  array_stats_function_id = "<the ID from above>"
...
the context manager (``with``)---an ``Executor`` does a lot of work in the background
to track tasks and futures, so it's good practice to encapsulate its usage in a
``with``-statement to clean it up even if other components break unexpectedly.
...
the `Compute Action Provider for Flows <../actionprovider>`_.
...
the endpoint side. For static workflows a manual approach is sufficient---ie,
administrators install packages as needed to the Python environment that the endpoint
is running in---but there are mechanisms that allow for more dynamic situations.
...
One way to  allows users to :doc:`specify a container
...
For static workflows where users always invoke the same function, administrators can simply install dependencies
directly into the endpoint's Python environment. In these cases administrators might want to ensure that **only** this known function is called, which can be achieved with a :ref:`function allow list <function-allowlist>`:
