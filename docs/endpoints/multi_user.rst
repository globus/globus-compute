...
This guide builds on the :doc:`endpoints` to introduce the additional features
that are required to enable multi-user endpoints.

Multi-user endpoints are installed the same way as regular endpoints. See
...
This biggest benefit of a multi-user endpoint setup is a lowering of the barrier for
...
The ``multi_user`` flag tells the web services to allow task submissions from
