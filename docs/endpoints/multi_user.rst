...
.. note::
   ...
   The only difference between a :abbr:`CEA (single-user Compute endpoint agent)` and a
   UEP is a semantic one for discussion to differentiate the two different starting
   contexts.  The CEA is started manually by a human, while a UEP will always have a
   parent MEP process.

.. , with the only difference from a standard :abbr:`CEA
.. (single-user Compute endpoint agent)` being that it was started not by a human at the
.. command line but on demand by the parent MEP.
.. From an administration perspective, a MEP is a configuration template as
.. :ref:`described in the previous section <endpoints_templating_configuration>` but with
.. an additional layer of *identity mapping*.  Before the interpolation step, the MEP will
.. first map the Globus Auth Identity passed along with the "start UEP" command to a local
.. username, and then become that user (c.f., ``uid``) before generating the concrete
.. configuration and finally starting the UEP.
...
come in.  (But see :ref:`mep-as-a-service` for automatic starting.)
...
N.B. this is example code highlighting the ``user_endpoint_config`` attribute on the
Executor; please generally consult the :doc:`../executor` documentation.
