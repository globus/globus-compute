...
While Globus Compute is in use on various systems around the world, getting to a
working configuration that matches the underlying system constraints and the
requirements of the site-administrator often takes trial and error.  Below are
...
The |GlobusMPIEngine|_ adds support for running MPI applications.  The following
...
Here is some Python that demonstrates how to compute the variables in the YAML
example above:
...
The following configuration example uses an Apptainer (formerly Singularity)
container on Midway.
...
system at Texas |nbsp| A |nbsp| & |nbsp| M |nbsp| (TAMU).  The configuration
...
The `Open Science Pool <https://osg-htc.org/services/open_science_pool.html>`_
is a pool of opportunistic computing resources operated for all US-associated
open science by the `OSG consortium <https://osg-htc.org/about/introduction/>`_.
...
`Stampede3 <https://tacc.utexas.edu/systems/stampede3/>`_ is a Dell technologies
and Intel based supercomputer at the `Texas Advanced Computing Center (TACC)
<https://tacc.utexas.edu/>`_.  The following snippet shows an example
