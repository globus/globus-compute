...
refer to :ref:`uep-conf` for an in-depth explanation of configuration features.
...
On the first invocation, the endpoint will emit a link to the console and ask for a Globus
Auth code in return. As part of this step, the Globus Compute web services will request
access to your `Globus Auth`_ identity and `Globus Groups`_. Your Globus Auth identity
will register as the endpoint owner and cannot be changed.
...
In some cases, it may also be helpful to run some setup during the user endpoint process,
before any workers start.  This can be achieved using the top‑level ``endpoint_setup``
config option:
...
In this case, ``container_cmd_options`` is treated as a string template, with the following two
strings replaced:
...
and enable HA functionality. Access control applies only to :doc:`multi-user endpoints
...
parent ``config.yaml``*.  In this setup, the web-service will not verify task-requested
...
to use can be changed in the manager endpoint process config via the ``amqp_port`` option:
...
#. The user acquires an endpoint ID (perhaps as shared by the administrator via an
...
#. The Compute web-service generates a unique identifier for a new user endpoint process
   ...
   This identifier is simultaneously stable and unique. In other words, changes to any of
...
#. The Compute web-service sends a message to the endpoint (via `AMQP`_), asking it to start
   a user endpoint process as the user that initiated the REST request and identified by the
...
#. The manager endpoint process calls ``fork()`` to ceate a new process.
...
#. The manager endpoint process validates the the user-defined variables against the JSON schema,
   if present, then renders the user endpoint configuration template.

#. The manager endpoint process calls ``exec()`` to launch the user endpoint process and passes the
   generated configuration over ``stdin``.
...
#. The web-services will see the check-in and then complete the original request to the
   SDK, accepting the task and submitting it to the now-running user endpoint process.
