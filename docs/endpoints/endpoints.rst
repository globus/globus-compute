...
.. note::
   Want more control over the endpoint configuration defaults? Both the :ref:`directory used
   to store endpoint data <config-dir>` and the :ref:`files used to populate the new
   endpoint directory <custom-config-files>` can be customized.
...
.. _custom-config-files:

Configuring With Custom Files
=============================
...
Now, the endpoint is configured with the custom display name, the user config template
is set up to render the ``endpoint_setup`` variable, and the user config schema is set
to only allow the ``endpoint_setup`` variable and no additional properties.
...
.. code-block:: console
   ...
   |         Endpoint ID          |   Status    |     Endpoint Name      |
   +==============================+=============+========================+
   | <...custom endpoint UUID...> | Initialized | Custom Config Endpoint |
   +------------------------------+-------------+------------------------+
   $ cd ~/.globus_compute/my_custom_endpoint/
   $ ls
   config.yaml  user_config_schema.json  user_config_template.yaml.j2
   $ cat config.yaml
   display_name: Custom Config Endpoint
   $ cat user_config_template.yaml.j2
   endpoint_setup: {{ endpoint_setup|default() }}
   $ cat user_config_schema.json
   {
      "$schema": "https://json-schema.org/draft/2020-12/schema",
      "type": "object",
      "properties": {
...
have clues as to what wrong.  Unfortunately, depending on setup and general access,
these logs may not be readily accessible, and so here is a list of known exit codes and
common causes of them:
