...
endpoint specified via during initial ``gce configure`` for single-user endpoints
or an autogenerated string in the form of
``uep.<MultiUserEndpointUUID>.<Generated.UEP.UUID>`` for identity-mapping endpoints.
...
the file ``endpoint.log`` in this directory.
