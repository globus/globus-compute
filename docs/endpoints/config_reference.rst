...
By default, User Endpoint configuration files and logs will be located in
``$HOME/.globus_compute/<EP_NAME>/`` where ``EP_NAME`` is either the name of the
...
directory for all user endpoints. The main User Endpoint logs will be appended
to the file ``endpoint.log`` in this directory.
