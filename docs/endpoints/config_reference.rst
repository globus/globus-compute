...
By default, User Endpoint logs configuration files will be located in
``~/.globus_compute/<UEP_NAME>/`` where ``UEP_NAME`` is either the name of the
endpoint specified via the ``configure`` command or an autogenerated string in the
form of uep.<MultiUserEndpointUUID>.<Generated.UEP.UUID>.

The main User Endpoint logs will be appended to the file ``endpoint.log`` in this
directory.

The directory path and the destination log file can be customized via the ``paths``
configuration in the ``user_config_template.yaml.j2`` jinja template.

.. code-block:: yaml
   :caption: ``~/.globus_compute/my_first_cluster_setup/user_config_template.yaml.j2``

   paths:
     gc_dir: {{ HOME ~ "custom_compute_dir" }}
     endpoint_log: /var/log/globus_compute_uep.log

   engine:
     ...

TBD more details and examples
* TBD Using {{ env.XYZ }} format
* TBD Using $HOME
* TBD Using pipe/redirect files0
