...
By default, User Endpoint(UEP) configuration files and logs will be located in
``$HOME/.globus_compute/<UEP_NAME>/`` where ``UEP_NAME`` is an autogenerated string
in the form of ``uep.<MultiUserEndpointUUID>.<Generated.UEP.UUID>``.
...
- The example above uses the reserved :ref:`_GC template variables <reserved-template-variables>`
  ...
  ``$HOME/.globus_compute/uep.<UUID>.<UUID>``

- The ``endpoint_log`` example collates all endpoint logs for a particular user
  to the same user-specific ``user_endpoints.log`` in ``/var/log/<username>``, instead
  of ``endpoint.log`` in each UEP's own individual directory.

- In the case of ``endpoint_log``, the custom value can be any path that the
  local user (UID) has write access to, including named pipes.
