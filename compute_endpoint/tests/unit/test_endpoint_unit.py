...
@pytest.mark.parametrize("manager_config", (pathlib.Path("config.yaml"), None))
@pytest.mark.parametrize("template_config", (pathlib.Path("template.yaml.j2"), None))
def test_configure_config_options(
    ...
    assert ("key: value" in dest_config.read_text()) == bool(manager_config)

    dest_template = Endpoint.user_config_template_path(conf_dir)
    assert dest_template.exists()
    assert ("key: {{ value }}" in dest_template.read_text()) == bool(template_config)
