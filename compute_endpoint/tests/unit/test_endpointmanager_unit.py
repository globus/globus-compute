...
def test_log_uep_pid_lines(
    ...
    # Loop exits on first non-zero pid
    mock_os.waitpid.side_effect = [(waitpid, 0), (1, 0)]
    ...
    exit_msg = f"Last child {waitpid} exited for CEP with PID"
    ...
    if waitpid != 0:
        ...
        assert mock_os.waitpid.call_count == 2
...
def test_title_changes_for_shutdown(
    ...
    *_, em = epmanager_as_root
    ...
    em._event_loop = noop
#XX    mocker.patch(f"{_MOCK_BASE}os")
