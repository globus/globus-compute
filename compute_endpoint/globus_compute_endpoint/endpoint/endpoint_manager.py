        ...
        if self.identity_mapper and not self._allow_same_user:
            ...
            if uname == p_uname or uid == os.getuid():
                raise InvalidUserError(
                    ...
                    "\n * removing privileges from the UID,"
                    "\n * removing the identity mapping configuration file"
                    f" ({self.identity_mapper.config_path}), or"
                    "\n * setting `force_mu_allow_same_user: true` in the MEP config"
                    f" ({Endpoint._config_file_path(self.conf_dir)})."
                    "\n\nDetails:"
