...
class EndpointManager:
    ...
    def cmd_start_endpoint(
        ...
        uid, uname = user_record.pw_uid, user_record.pw_name
        ...
        exec_args = [sys.executable, sys.argv[0], "start_user_endpoint"]
        ...
        try:
            ...
            stdin_data = json.dumps(ue_data, separators=(",", ":"))
            os.write(stdin_w, stdin_data.encode())
            os.close(stdin_w)
            ...
            pid = os.posix_spawn(
                sys.executable, exec_args, {}, file_actions=file_actions
            )
            os.close(stdin_r)

        except Exception as e:
            exc_type = type(e).__name__
            log.error(f"Unable to create child process: ({exc_type}) {e}")
            raise
        finally:
        ...
        log.info(f"Created new user endpoint (pid: {pid}) [{proc_args_s}]")
