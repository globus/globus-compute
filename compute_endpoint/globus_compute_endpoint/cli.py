...
@optgroup.option(
    ...
    type=click.Path(exists=True, dir_okay=False, path_type=pathlib.Path),
    default=None,
    help="Specify a custom user configuration template (user_config_template.yaml.j2)",
)
@optgroup.option(
    "--schema-config",
    type=click.Path(exists=True, dir_okay=False, path_type=pathlib.Path),
    default=None,
    help="Specify a custom user configuration schema (user_config_schema.json)",
)
@optgroup.option(
    "--user-env-config",
    type=click.Path(exists=True, dir_okay=False, path_type=pathlib.Path),
    default=None,
    help="Specify a custom user environment configuration (user_environment.yaml)",
)
@optgroup.option(
    "--id-mapping-config",
    type=click.Path(exists=True, dir_okay=False, path_type=pathlib.Path),
...
@common_options
@handle_auth_errors
@click.option("--log-path", type=str, required=False, default=None)
def start_user_endpoint(*, ep_dir: pathlib.Path, log_path: str | None, **_kwargs):
    with open("/tmp/my.log", "a") as f:
        f.write(f"ep_dir is {ep_dir} and log_path is {log_path}\n")
    state = CommandState.ensure()
    custom_log_path = None
    if log_path:
        custom_log_path = pathlib.Path(log_path)
    _start_user_endpoint(
        ep_dir=ep_dir,
        endpoint_uuid=state.endpoint_uuid,
...
def _start_user_endpoint(
    *,
    ep_dir: pathlib.Path,
    endpoint_uuid: str | None,
    custom_log_path: pathlib.Path | None = None,
):
    os.umask(0o077)
    state = CommandState.ensure()

    if isinstance(custom_log_path, pathlib.Path):
        # This will override the default `ep_dir/endpoint.log` in ensure_log_path
        os.environ[LOG_PATH_ENV] = str(custom_log_path.resolve())
        log.info(f"Custom logging file path set to {os.environ[LOG_PATH_ENV]}")
