...
@click.option(
    ...
    default=None,
    help=(
        "Override the default behavior of creating an identity mapping config if the "
        "current user is privileged. --multi-user always creates an identity mapping "
        "config, --no-multi-user never creates an identity mapping config. If both "
        "are present, the one specified last wins."
    ),
)
@click.option(
    "--high-assurance",
