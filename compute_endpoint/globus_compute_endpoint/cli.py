...
def configure_endpoint(
    *,
    ...
    if manager_config:
        if core_config:
            warnings.warn(
                "Both --manager-config and --core-config were provided;"
                " --manager-config is deprecated and will be ignored.",
                UserWarning,
                stacklevel=2,
            )
        else:
            warnings.warn(
                "--manager-config is deprecated; use --core-config instead.",
                DeprecationWarning,
                stacklevel=2,
            )
            # Warn but alias the older argument to the new one
