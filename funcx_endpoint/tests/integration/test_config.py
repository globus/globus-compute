...
if __name__ == "__main__":
    ...
    max_workers = (
        ""
        if config.max_workers_per_node == float("inf")
        else f"--max_workers={config.max_workers_per_node}"
    )
    ...
    launch_cmd = (
        "funcx-worker {debug} {max_workers} "
        "-c {cores_per_worker} "
        "--poll {poll_period} "
        "--task_url={task_url} "
    ...
    print(f"Launch command: {config.launch_cmd}")
