...
class TaskQueue(object):
    ...
    def __init__(self,
                 ...
                 SNDTIMEO = None,
                 ironhouse: bool = True,
                 # client_keys_dir: str = os.path.abspath('.curve'),
                 client_keys_dir: str = '/tmp',
                 mode: str = 'client'):
        """
        Parameters
        ----------
        ...
        ironhouse: Bool
           ...
           client keys to be available on the server in the client_keys_dir.
        """
        if zmq_context:
        ...
        os.makedirs(self.client_keys_dir, exist_ok=True)
        logger.info("Client key store is at {os.path.abspath(self.client_keys_dir)}")
    ...
    def add_client_key(self, endpoint_id, client_key):
        ...
        if self.ironhouse:
            ...
            with open(os.path.join(self.client_keys_dir, f'{endpoint_id}.key'), 'w') as f:
                f.write(client_key)
            try:
                self.auth.configure_curve(domain='*', location=self.client_keys_dir)
            except:
                logger.exception("Failed to load keys from {self.client_keys_dir}")
        return

    def setup_server_auth(self, secret_keys_dir='/tmp'):
    ...
    def setup_client_auth(self, secret_keys_dir='/tmp'):
        ...
        server_public_file = os.path.join(secret_keys_dir, "server.key")
