...
class GlobusTransferClient:
    ...
    def __init__(self,
        ...
        transfer_authorizer = self.native_client.get_authorizers_by_scope(
            requested_scopes=scopes)[transfer_scope]
    ...
    def transfer(self, src_ep, src_path, basename, recursive=False):
        ...
        try:
            ...
            logger.info("Task info: {}".format(task))
    ...
    def get_event(self, task):
        task_id = task['task_id']
        events = self.transfer_client.task_event_list(task_id, num_results=1, filter='is_error:1')
        try:
            event = events.data[0]['details']
            # logger.info("Got globus transfer event for task {}: {}".format(task_id, event))
            return event
        except IndexError:
            logger.debug("No globus transfer error for task {}".format(task_id))
            return
        except Exception:
            logger.exception("Got exception when fetching globus transfer error event for task {}".format(task_id))
            return

    def cancel(self, task):
        task_id = task['task_id']
        res = self.transfer_client.cancel_task(task_id)
        logger.info("Canceling task {}, got message: {}".format(task_id, res))
        if res['code'] != 'Canceled':
            logger.error('Could not cancel task {}. Reason: {}'
                         .format(task_id, res['message']))
