...
def test_start_endpoint_blocked(
    ...
    service_addr = "http://api.compute/"
    gcc = globus_compute_sdk.sdk.client.Client(
        funcx_service_address=service_addr,
        do_version_check=False,
        login_manager=mocker.Mock(),
    )
    wc = WebClient(base_url=service_addr)
    gcc.web_client = wc
...
def test_stop_remote_endpoint(
    mock_get_id, mock_get_conf, mock_get_config, mock_stop_endpoint
):
