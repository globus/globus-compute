...
_COMPUTE_HOME = os.path.join("~", ".funcx")


class Client:
    """Main class for interacting with the Globus Compute service

    Holds helper operations for performing common tasks with the service.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        results_ws_uri: str
            ...
            DEPRECATED - use globus_compute_sdk.Executor instead.
    ...
    def version_check(self, endpoint_version: str | None = None) -> None:
        ...
        if endpoint_version is not None:
            compare_versions(
                endpoint_version,
                min_ep_version,
                package_name="globus-compute-endpoint",
            )
    ...
    def _update_task_table(self, return_msg: str | t.Dict, task_id: str):
        ...
        if not pending:
            ...
            if "result" in r_dict:
                try:
                    r_obj = self.serializer.deserialize(r_dict["result"])
                except Exception:
    ...
    def batch_run(self, batch) -> t.List[str]:
        ...
        for result in r["results"]:
            ...
            if not (200 <= result["http_status_code"] < 300):
                ...
                # Checking for 'Failed' is how we originally checked for errors.
        ...
        if self.asynchronous:
            ...
            for task_id in task_uuids:
                task = ComputeTask(task_id)
                asyncio_task = self.loop.create_task(task.get_result())
                asyncio_tasks.append(asyncio_task)

                self.ws_polling_task.add_task(task)
    ...
    def get_containers(self, name, description=None):
        """
        Register a DLHub endpoint with the Globus Compute service and get the
