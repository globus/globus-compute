...
def get_diagnostic_commands(log_bytes: int, home_dir: str | None = None):
    ...
    if home_dir:
        specified_path = Path(home_dir)
        if specified_path.exists() and specified_path.is_dir():
            gc_home_dir = specified_path
        else:
            # Should we default to gc_home_dir if the specified path doesn't
            # exist?
            raise ValueError(
                f"Specified path '{specified_path.absolute()}' is not a valid "
                f"directory, will default to {gc_home_dir} without parameter"
            )
