...
def get_recent_endpoints(base_dir: Path, ep_names: list | None = None) -> list:
    ...
    return [e[1] for e in sorted(ep_list_by_ts)]


def get_diagnostic_commands(
    log_bytes: int,
    config_dir: str | None = None,
    ep_names: list | None = None,
    recent_only: int = -1,
):
    ...
    if "is not installed" not in ep_install_dir:
        ...
        if log_bytes > 0:
            ...
            if recent_only > 0 or ep_names:
                log_paths = []
                if recent_only > 0:
                    ep_paths = get_recent_endpoints(gc_base_dir, ep_names)[:recent_only]
                else:
                    ep_paths = []
...
def run_all_diags_wrapper(
    ...
    :param ep_names:           A list of endpoint directory names to gather logs
...
def do_diagnostic_base(diagnostic_args):
    ...
    parser.add_argument(
        ...
        help=(
            ...
            " Multiple directories can be specified e.g. -l ep1 ep2"
        ),
    )
    parser.add_argument(
        "-r",
        ...
        type=int,
        help=(
            "Gather endpoint logs and configurations from only the most recent"
            " # endpoints, sorted by most recently modified endpoint.log"
        ),
    )
